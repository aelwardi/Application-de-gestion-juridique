...
const acceptCounterSuggestion = async (suggestionId: string) => {
  const confirmed = await confirmModal.confirm({
    title: 'Accepter la contre-proposition',
    message: 'Accepter cette contre-proposition et créer le rendez-vous ?',
...
const rejectCounterSuggestion = async (suggestionId: string) => {
  ...
  try {
    ...
    if (response.success) {
      toast.success('Contre-proposition refusée');
      await loadSuggestions();
    }
  } catch (error) {
              ...
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  ...
                  <div v-if="suggestion.status === 'countered'" class="mt-4 bg-blue-50 border-l-4 border-blue-400 p-3 rounded">
                    <p class="text-sm text-blue-800">L'avocat a proposé un autre créneau - Consultez l'onglet "Contre-propositions reçues"</p>
                  </div>
                </div>
              </div>
            </div>
