...
import type { ClientWithUser, ClientStats, ClientCase, ClientAppointment, ClientDocument } from '~/types/client';
...
const loadClientData = async () => {
  ...
  try {
    ...
    if (client.value) {
      stats.value = await getClientStats(client.value.userId);
    }
  } catch (error) {
...
watch(activeTab, async (newTab) => {
  ...
  if (newTab === 'cases' && cases.value.length === 0) {
    ...
    const result = await getClientCases(client.value.userId, 20, 0);
    cases.value = result.data;
    loadingCases.value = false;
  } else if (newTab === 'appointments' && appointments.value.length === 0) {
    loadingAppointments.value = true;
    const result = await getClientAppointments(client.value.userId, 20, 0);
    appointments.value = result.data;
    loadingAppointments.value = false;
  } else if (newTab === 'documents' && documents.value.length === 0) {
    loadingDocuments.value = true;
    const result = await getClientDocuments(client.value.userId, 20, 0);
